...
<%@page import="MemV02.MemDTO"%>
<%@page import="MemV02.MemDAO"%>
...
/*   	MemDTO memDTO = new MemDTO();
	...
	for(int i = 0; i< memDtoL.size();i++){
		memDTO = memDtoL.get(i); */
 		List<MemDTO> dtoL = (List)request.getAttribute("dtoL");
		if(dtoL != null){
		for(int idx=0;idx<dtoL.size();idx++){
			MemDTO memDTO = dtoL.get(idx);
%>
        	<tr>
		...
		response.sendRedirect(vUrl+"/ControllerMemV02.do?category=memSel");
		}%>
        </table>
        </div>
        </section>
